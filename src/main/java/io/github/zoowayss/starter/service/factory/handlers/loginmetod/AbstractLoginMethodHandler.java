...
public abstract class AbstractLoginMethodHandler implements LoginMethodHandler {
    ...
    public String doLogin(LoginReq loginReq) {
        ...
        if (Objects.isNull(currentUser)) {
            // 自动注册
            long userId = ((AbstractLoginMethodHandler) AopContext.currentProxy()).registerNewUser(loginReq);
        ...
        if (currentUser.isDeleted()) {
            throw new BusinessException(ResultCodeEnum.PASSPORT_ERROR_USER_DELETED);
        }

        if (!matchPassword(loginReq, currentUser)) {
            throw new BusinessException(ResultCodeEnum.PASSPORT_ERROR_PWD_INCORRECT);
        }
